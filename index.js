...
function filterStudentsByGrade(students, grade) {
  ...
  console.clear();
  console.info("Завдання: 1 ==============================")
  const filterGrade = students.filter((student) => student.grade === grade);
  filterGrade.forEach((student) => {console.log (student)
...
function logArrayElements(arr) {
	...
	arr.forEach((index, value) => {console.log("Елемент", value + ":", index)});
...
function compareArrays(arr1, arr2) {
  ...
  let sortFn = (elem1, elem2) => elem1 - elem2;
  let sortarr1 = arr1.sort(sortFn);
  let sortarr2 = arr2.sort(sortFn);
  let result = JSON.stringify(sortarr1) === JSON.stringify(sortarr2);
...
function validateUserInput(userInput) {
	...
	let goodPassword = userInput.password.length < 8;
	console.assert(!goodPassword, "Попередження: пароль має бути довшим за 8 символів!");
...
function calculateTotalPrice(products) {
	const counterLabel = "calculateTotalPrice";
...
function countVowelsAndConsonants(word) {
	...
	for (const letter of word) {
		let lit = letter.toLowerCase();
		if (a.includes(lit) ) {
			console.count("Голосні")
		}
		else console.count("Приголосні")
	}
